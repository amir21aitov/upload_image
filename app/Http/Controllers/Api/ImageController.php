...
class ImageController extends Controller
{
    ...
    {
        $image = $this->imageService->upload($request->file('image'), $request->user(),);

        return response()->json(['data' => new ImageResource($image)], Response::HTTP_CREATED);
    }

    public function index(ImageIndexRequest $request): JsonResponse
    {
        $paginator = $this->imageService->listForUser(
            $request->user(), ImageFilterDTO::fromRequest($request),
        );
